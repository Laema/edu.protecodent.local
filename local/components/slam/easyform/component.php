...
if ($request->isPost() && $arParams['FORM_ID'] == $request['FORM_ID']) {
    ...
    if (empty($arMessageError) && $arParams['ENABLE_SEND_MAIL']) {
        ...
        //check select email
        if (isset($selectEmailField) && isset($arPostFields[$selectEmailField])) {
                foreach($arParams['CATEGORY_'.$selectEmailField.'_VALUE'] as $key => $customValue) {
                   $arSelectEmailsValues[$customValue] = $arParams['CATEGORY_'.$selectEmailField.'_VALUE_EMAIL'][$key];
                }

                if($arSelectEmailsValues[$arPostFields[$selectEmailField]] && strlen($arSelectEmailsValues[$arPostFields[$selectEmailField]]) > 0 && check_email($arPostFields['EMAIL'])){
                    $arServiceFields['EMAIL_TO'] = $arSelectEmailsValues[$arPostFields[$selectEmailField]];
                }
        }
    ...
    if (!empty($arMessageError)) {
        $result = array(
            ...
            'captcha_errors' => implode("</br>", $errors),
