...
HTML;
?>
<ol style="display: none" class="file-placeholder-tbody webform-field-upload-list webform-field-upload-list-<?= $arParams["MULTIPLE"] == "Y" ? "multiple" : "single" ?><?
...
<? if ($arParams["ALLOW_UPLOAD"] != "N") {
    ?>
    <label class="input input_file" id="mfi-<?= $arParams['CONTROL_ID'] ?>-button">
    <?/*
    ...
    if ($arParams["SHOW_AVATAR_EDITOR"] == "Y" && $arParams["ALLOW_UPLOAD"] == "I") {
        ...
        ?><input type="file" onchange="uploadFile(this)"
                 id="file_input_<?= $arParams['CONTROL_ID'] ?>" <?= $arParams["MULTIPLE"] === 'Y' ? ' multiple="multiple"' : '' ?> /><?
    }
    ?>
        <div class="input_file__icon">
            <svg width="20" height="20">
                <use xlink:href="<?=SITE_TEMPLATE_PATH?>/assets/images/sprite.svg#icon-file"></use>
            </svg>
        </div>
